...
const userbase =
  typeof window !== "undefined" ? require("userbase-js").default : null
...
class Todo extends React.Component<{ user: userbase.UserResult }, ITodoState> {
  ...
  componentDidMount() {
    userbase
      .openDatabase({
        databaseName: "todos",
        changeHandler: this.handleDBChanges,
      })
      .catch((error: IError) => {
  ...
  handleCompletionChange(
    ...
    userbase
      .updateItem({ databaseName: "todos", item: updatedTodo, itemId: itemId })
      .catch((error: IError) => {
        this.setState((state: ITodoState) => {
          return {
            ...state,
  ...
  deleteTodo(
    ...
    userbase
      .deleteItem({ databaseName: "todos", itemId: itemId })
      .catch((error: IError) => {
        this.setState((state: ITodoState) => {
          return {
            ...state,
  ...
  handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (this.state.addTodoForm.changed && this.state.addTodoForm.valid) {
      const todoName: string = this.state.addTodoForm.getFormField("todoName")
        .value
      userbase
        .insertItem({
          databaseName: "todos",
          item: { name: todoName, completed: false },
        })
        .then(() => {
  ...
  render() {
    return (
      <Layout user={this.state.user}>
        <SEO title="Todos" />
        <h1 className="m-0 mb-3 text-4xl">
          Welcome,{" "}
        ...
        <h2 className="m-0 text-2xl">Bidding List</h2>
        ...
        <div className="mt-4">
          {this.state.todos.map((todo: userbase.Item, index: number) => {
            return (
              ...
              >
                <button
                  className="btn-icon bg-red-800 text-white"
                  onClick={e => this.deleteTodo(e, todo.itemId, index)}
                >
                  X
                </button>
                <input
                  ...
                  onChange={e => {
                    this.handleCompletionChange(e, todo.item, index)
                  }}
                />
                <label
                  className={`mb-0 ml-4 ${
                    todo.item.completed ? "line-through" : ""
                  }`}
                  htmlFor={todo.itemId}
                >
                  {" "}
        ...
        <form className="max-w-sm" onSubmit={this.handleSubmit}>
          <div className="mb-4">
            ...
            <p
              className={`error-msg ${
                this.state.addTodoForm.getFormField("todoName").error !==
                Validators.none
                  ? "show"
                  : ""
              }`}
